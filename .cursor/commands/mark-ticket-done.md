...
For token retrieval, stable IDs, and `curl` patterns to update the Project `4` `Status` field, see `docs/process/github-project-tracking.md` → **GitHub GraphQL API**. If the mutation fails, report a **Blocked requirement** for Project sync; issue close and Epic body updates may still proceed independently.
