...
- **Refinamiento S13 (2026-06-16):** **no toda** mezcla neutra en oklch drifta. La auditoría final marcó `StoreAvatar` (`color-mix(in oklch, var(--text-muted) 12%, var(--surface-elevated))`) como sospechosa, pero la verificación en preview (light) mostró que **NO drifta**: ambos tokens caen en el hue Velvet 285, así que oklch da `oklch(0.891 0.0167 285)` = idéntico a oklab. El drift solo aparece cuando se mezcla contra `transparent` (hue→0) o entre tokens de **hue ambiguo / chroma ~0**. **Lección:** no asumas drift por la mera forma `oklch + token neutro` — verificá el hue computado. Pero la convención sigue siendo **blanket** (neutrales en oklab) porque no es evidente en autoría cuál instancia derivará, y ahora hay una **guardia automática**: `src/test/design-token-guard.test.ts` falla la suite ante cualquier `color-mix(in oklch, var(--<neutral>))` (+ clases Tailwind theme-blind en `.tsx`). Cero-dep, por ADR 0010.
