...
> **Estado actual:** S1-S6 ✅ done · M01-M06 ✅ done · S5.2 + S5.3 ✅ done · S7 Fase A ✅ done · **S7 Fase B (Orders end-to-end) ✅ done · S8 Fase A (Settings demo + specs + patrón Chip Eyebrow cross-module) ✅ done**. Próxima: **S8 Fase B — Settings implementación + uplift visual cross-module (order-detail, store-detail)**. **Plan revisado post-S4** (ver [`methodology.md`](./methodology.md)): cada sesión tiene **Fase A (docs) + Fase B (implementación inmediata)** con gate humano en el medio. Todo va a branch `redesign`. Dashboard fuera de scope. Última sesión (S14) reemplaza `docs/design/` con el sistema final.
>
...
>
> **Próxima sesión: 🟡 S8 Fase B — Settings implementación + uplift visual cross-module** — conversación nueva. Implementación React/Next del módulo Settings completo + extensión del patrón Chip Eyebrow + Top-Accent a las 4 variantes de `order-detail` y 3 de `store-detail` per PLAYBOOK §9.17 vocabulario congelado. Prerequisitos: actualizar FRD-07 con propuestas P-S8-01 a P-S8-08 (idioma visible, two-path currency, deferred features) — puede hacerse en mini-sesión paralela o como parte de Fase B.
>
...
| 08  | Pantalla Ajustes (Profile / Account / Preferences + modales adaptive) | módulo lite (1 pantalla + tabs) | 🟡 next (Fase A iterativa con HTML-first; insertada en roadmap 2026-05-17 por decisión humana) |
| 09  | Módulo Entregas (crear / lista / detalle)                             | módulo                          | ⏳ pending (renumerada desde S8)                                                               |
...
- Sesión vigente: **S8 Fase B — Settings implementación + uplift visual cross-module** 🟡 próxima. S8 Fase A cerrada 2026-05-18 con demo completo + specs spec-complete + patrón Chip Eyebrow + Top-Accent documentado en PLAYBOOK §9.17 (cross-module: Settings origen, `order-detail` y `store-detail` candidatos para uplift). Fase B implementa el módulo Settings React/Next y aplica el uplift visual al detalle de pedidos y tiendas en una sola sesión.
