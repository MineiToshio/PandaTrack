...
| `lib/data/orders/orderQueries.ts`        | `getOrdersList`: sort default `orderDate: "desc"` (era `"asc"`); agregar `pendingFxCount` vía `Promise.all` con `prisma.order.count({ where: { userId, needsExchangeRateUpdate: true } })`                                                                  |
...
2. **P-S7-02** — Sort opciones adicionales (Tienda A–Z, % Pago: menor, Total: mayor): ¿se implementan en Fase B o se simplifica el select a solo 2 opciones (Más recientes / Más antiguas)?
