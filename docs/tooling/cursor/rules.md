...
- `design-system-playbook.mdc`: Read before implementing or refactoring ANY UI in `src/`. Points to `docs/redesign/PLAYBOOK.md` which distills accionable rules from operational lessons of the redesign subproject (component canonical sources, token conventions, canonical patterns, anti-patterns, mandatory pre-implementation workflow, self-audit checklist).
