...
- `error-handling-validation.mdc`: Use when adding validation, boundary parsing, error handling, or safe logging. Also carries the domain-date contract: every calendar-day field is declared with `domainDateSchema` (never a bare `z.coerce.date()`), and the client normalizes with `toDomainDate` before a `Date` crosses a Server Action boundary.
...
| `error-handling-validation.mdc`        | Zod, expected errors, logging; domain dates must use `domainDateSchema`                                  | Validation, parsing, or error handling changes; any calendar-day field                                                                                                                   |
