...
- `project-structure.mdc`: Use when deciding where files should live or when adding new route-level or shared modules, including `_components/share/` folders for route-subtree reuse and promotion from child pages when reuse appears, and when deciding where a new domain's Prisma query/mutation module goes (`src/lib/data/<domain>/` vs legacy `src/queries/`, see ADR 0015).
