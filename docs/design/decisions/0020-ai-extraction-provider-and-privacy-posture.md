...
- **Server time budget (found in production 2026-08-28, and the reason extraction never worked there).** Extraction is the one action in this product that outlives the platform's default function timeout. Measured against the live API with three real chat screenshots, the provider call alone takes ~24 seconds and the whole submission ~45. The hosting default is **10 seconds** on Hobby, so without an explicit budget the function is killed mid-call on **every** submission: the collector sees a generic failure, and because the ledger writes its reservation _before_ the call, each killed attempt orphans a `PENDING` row that keeps counting against their monthly photo bag for a read they never received. The fix is a pair of numbers that must move together: the intake route declares `export const maxDuration = 60` (Hobby's ceiling; Pro would allow more), and `EXTRACTION_TOTAL_BUDGET_MS = 55_000` caps the engine's whole provider phase, retries and backoffs included, so it always returns a typed failure the ledger can settle instead of being killed with the reservation open. The retry count alone could not do this: three full-length attempts are 30s + 0.7s + 30s + 0.7s + 30s ≈ 92 seconds, which no plan tier here allows. Both bounds are live, whichever is reached first, so fast failures still get all three attempts. Pinned by `src/test/image-intake-config-guard.test.ts`, which fails if the export is missing, exceeds the plan ceiling, or drops below the engine's budget.
