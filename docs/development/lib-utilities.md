...
| `src/lib/authVerificationEmail.ts` | Auth verification email business logic (locale resolution, i18n templates, HTML/text composition). |
...
| `src/lib/kit.ts`                   | Kit.com API integration (subscriber/tag operations).                                               |
...
| `src/lib/resend.ts`                | Generic Resend email sender utility (provider-level API call).                                     |
