...
Implement payment persistence and payment mutation rules so collectors can track what has been paid, what remains, and correct mistakes by deleting payments when needed. This slice delivers the `addPayment` and `deletePayment` mutations with their atomic transaction contracts, balance guardrails, derived summary calculations, and history entry creation. The payment panel UI is built in [BP-02 · WO-05](../../bp-02-order-workspace-and-list-experience/work-orders/wo-05-order-detail-view-private-note-payments-panel-and-action-menu.md).

## In Scope

- `addPayment` Server Action: validates amount and date, checks remaining balance atomically, persists `OrderPayment`, appends `PAYMENT_ADDED` history entry
- `deletePayment` Server Action: verifies ownership, deletes `OrderPayment`, appends `PAYMENT_DELETED` history entry
...
- `addPayment` must execute `[read current balance, insert OrderPayment, insert OrderHistory]` inside a single `prisma.$transaction`. The balance check reads the aggregate `SUM(amount)` for `{ orderId, userId }` within the transaction to prevent TOCTOU races.
- `deletePayment` must execute `[delete OrderPayment, insert OrderHistory]` atomically. No balance race check is needed for delete.
...
- `OrderPayment` schema, `PAYMENT_ADDED` and `PAYMENT_DELETED` `OrderHistoryEventType` values, `orderPaymentMutations.ts` module path, and `userId` on `OrderPayment` are established by [BP-01 · WO-01](./wo-01-currency-catalog-order-identifiers-and-persistence-contracts.md).
