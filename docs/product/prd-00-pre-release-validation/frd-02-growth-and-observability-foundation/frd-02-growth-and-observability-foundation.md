...
- `POSTHOG_EVENTS` constants in `src/lib/constants.ts` cover 111 events across 8 categories; `FEATURE_FLAGS` in the same file holds runtime PostHog feature flags
...
`src/lib/constants.ts` `POSTHOG_EVENTS` — **111 events across 8 categories**. This is a snapshot for product traceability; the code is authoritative if it diverges. The naming enforcement rules (declarative `data-ph-event` / `data-ph-props` for simple clicks, two-level `module_action` naming, snake_case non-PII properties) are defined in `.cursor/rules/posthog-events.mdc`.

| Category     | Count | Events                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------ | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LANDING`    | 7     | `hero_cta_clicked`, `banner_cta_clicked`, `header_cta_clicked`, `mobile_menu_opened`, `mobile_menu_nav_clicked`, `faq_item_toggled`, `social_link_clicked`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
**Defined but unwired (no callsites as of 2026-06-16):** six event names exist in `POSTHOG_EVENTS` but are never captured anywhere in `src/` — `STORE.REVIEW_WRITE_CLICKED` (`store_review_write_clicked`), `ORDER.CREATE_DISCARDED` (`order_create_discarded`), `ORDER.VIEW_STORE_CLICKED` (`order_view_store_clicked`), `ORDER.DETAIL_MORE_MENU_OPENED` (`order_detail_more_menu_opened`), `ORDER.LIST_PAGE_CHANGED` (`orders_list_page_changed`), and `APP_SHELL.MASCOT_SHOWN` (`app_shell_mascot_shown`). They are counted in the totals above but emit nothing until a callsite is added. |
