...
- The detail view uses a two-column layout on `lg` and above: **left (main)** column — `OrderDetailHero`, then the **items** subcard, then the **read-only order history** as a `CollapsibleSubcard` matching the items subcard. **Right** column (sticky) — **payment summary + payment list/add form** (`OrderPaymentsAsideCard`), then the actions card, then the **private note**. On smaller breakpoints the grid stacks: **hero → items → payments → note → actions** (no sticky rail; the sticky action bar owns the primary CTA), and **history is not rendered below `lg`**. See [`WO-05`](work-orders/wo-05-order-detail-view-private-note-payments-panel-and-action-menu.md).
