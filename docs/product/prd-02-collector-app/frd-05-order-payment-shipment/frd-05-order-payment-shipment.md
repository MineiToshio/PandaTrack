...
- `FR-05-43`: A payment's own amount must never exceed what the collector currently owes its store in the payment's currency (`StoreDebtRow.debtMinor` = Σ `totalCost` of the store's non-cancelled orders − Σ the store's payments, in that currency); exceeding it is refused (`STORE_DEBT_EXCEEDED`) rather than silently accepted as a further credit. Independently, the sum of everything declared in the sheet must never exceed the payment's own amount (`ALLOCATION_SUM_EXCEEDS_PAYMENT`), and each order/product line must never exceed that order's/product's own remaining balance (`EXCEEDS_BALANCE` / `EXCEEDS_ITEM_BASE`).
...
- **(2026-08-08)** a store's debt (`Σ totalCost` of its non-cancelled orders `− Σ` its payments, per currency) is **not clamped at zero**: a negative value is real money the store holds on the collector's behalf and renders as "a favor" (green), never folded into or confused with the unrelated photo-quota "crédito" veto (`docs/product/glossary.md`)
