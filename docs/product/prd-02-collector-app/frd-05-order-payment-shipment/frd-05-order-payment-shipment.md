...
| Cancel                | confirmation modal (`tone-warning`; optional reason; keep/remove payments choice when the order has payments) | optimistic confirmation: the modal closes on submit and the cancelled state shows immediately; rolls back on failure. **Payments are kept by default (surfaced as lost on the dashboard) or removed on the remove choice.** | stays on detail                                    |
...
| any non-`CANCELLED` | cancel                                    | `CANCELLED`                        | **payments kept by default, or removed on the modal's remove choice** (`BR-05-15`); `cancellationReason` stored; `ORDER_CANCELLED` history; blocked when any item has a live delivery link |
| `CANCELLED`         | reactivate                                | `OPEN`                             | `cancellationReason` cleared; `ORDER_REACTIVATED` history; **kept payments remain attached/visible; payments removed at cancel time stay gone**                                            |
...
- a cancelled order may be reactivated to `OPEN`; payments kept at cancel time remain attached and visible after reactivation, while payments the collector chose to remove at cancel time stay gone (`BR-05-15`)
