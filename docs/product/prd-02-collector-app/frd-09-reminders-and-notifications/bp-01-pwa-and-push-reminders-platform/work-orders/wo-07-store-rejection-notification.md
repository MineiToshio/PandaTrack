...
- Any report-driven signal. Only a transition into `StoreStatus.REJECTED` triggers this notification. A store accumulating open reports shows a derived public notice with no state transition behind it (PRD-02, FRD-04 `FR-04-43`), so it never notifies the creator.
