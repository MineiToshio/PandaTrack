...
- **`RankEmblem` (`src/components/core/RankEmblem.tsx`) is new**, and is registered in `docs/design/components.md`. It follows the same choice `MedalStage` made for medal art: one sober placeholder for all ten ranks (plate, band ring, the rank numeral in the mono face) until rank artwork exists, rather than ten guessed crests. It draws no animation of its own, which is what lets the dashboard instance be calm without an override.
