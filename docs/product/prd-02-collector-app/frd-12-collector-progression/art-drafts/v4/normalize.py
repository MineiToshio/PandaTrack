...
def normalize(src, dst):
    ...
    scale = min(scale_bbox, scale_cap)
    capped = scale_cap < scale_bbox
    ...
    print(
        ...
        f"radius={r_final:.0f}/{MAX_RADIUS:.0f} limited_by={limit} "
        f"holes_filled={holes} despilled={spill} corners_max_alpha={corners} "
        f"mode={check.mode}"
    )
