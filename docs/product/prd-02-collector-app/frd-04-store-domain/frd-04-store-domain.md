...
- Store-level payments on the store detail page (`FR-04-55`, `FR-04-56`, `FR-04-58`, `FR-04-59`, 2026-08-08, extended 2026-08-10): a per-currency payment progress block in the aside (which replaced the flat "Deuda pendiente" row, measures the pedidos activos rather than the store's whole history, and sits under a "Total facturado" that now names its cancelled slice), a "Registrar pago" entry point into the shared store payment sheet, and a "Pagos a esta tienda" card in the main column listing the collector's own payments to this store, each row naming what the payment covers, with a read-only breakdown for multi-declaration payments and a delete. The underlying money model (declared allocations, the store payment sheet itself) is owned by [FRD-05](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md) (`FR-05-42`–`FR-05-44`, [ADR 0025](../../../design/decisions/0025-store-level-payments-declared-allocations.md)); this FRD owns only the store-detail entry points and presentation.
...
- `FR-04-55`: The store detail aside's "Resumen" card must show the viewer's own standing with this store as a **payment progress block, one per currency** (`FR-04-59`), which replaces the former flat "Deuda pendiente" row. _(Rewritten 2026-08-10, amended the same day.)_ The old row printed `S/ 0.00` on 104 of the collector's 120 stores, spending a line to say nothing, and gave the store/currency pairs that do owe something no sense of how far along they were. The "Total facturado" row above it **stays**, and gains a conditional "Cancelados {amount}" line directly beneath it (`FR-04-59`) whenever that total differs from the store's committed total. The block and the rows are omitted entirely, together with the rest of the "Resumen" card, when the viewer has never ordered from or paid this store.

  **The card carries two scopes, and every figure says which one it is in.** "Pedidos anotados" and "Total facturado" are lifetime and count cancelled orders; "Pedidos activos" and the progress bar cover only the pedidos activos ([glossary](../../glossary.md)). That pairing is deliberate: the bar measures the money of exactly the set the row above it counts, and no figure in the card is left to be read as a total when it is a slice. What must never come back is two figures that both look like totals and are computed differently, which is the contradiction the "Cancelados" line was added to end.
...
- **`FR-04-57` removed (2026-08-10):** the "Acciones" card briefly offered a "Ver mis pedidos en esta tienda" link into `/orders?view=store`, but that view is the cross-store grouped list (`getPendingProductsByStore(userId)` takes no store filter), not a view scoped to this store, so the link's promise did not match its destination. Pulled rather than fixed; the "Resumen" card's existing "Ver pedidos vinculados" link (`?store={storeId}`) already covers the store-scoped case.
- `FR-04-58`: The store detail **main column** must offer a "Pagos a esta tienda" card listing every `StorePayment` the viewer has made to this store, newest first. The card renders nothing when the viewer has never paid this store. Each row offers a delete action gated by a destructive confirmation modal; confirming it removes the entire `StorePayment` and every `PaymentAllocation` declared against it, not only a slice of it, because a payment belongs to exactly one store regardless of how many orders it is declared against. This is the only surface that can reach and remove a payment that carries no allocation ("on account") or an undeclared remainder, complementing the order-scoped payment delete ([FRD-05 · `BR-05-10`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#business-rules)), which removes only one order's slice of a payment shared across orders. _(Extended 2026-08-10.)_ Each row must answer "what was this payment for?" without any expanding:

  - **Coverage.** Beside the date and the amount, the row names what the payment covers: `{order reference} · {product}` for a product-level declaration, `{order reference} · Todo el pedido` for an order-level one, `{order reference} · {n} productos` when several lines share one order, `{n} pedidos` when they span several, and the existing "Sin asignar {amount}" chip when part or all of the payment is undeclared. The order reference links to that order. A declaration pointing at a **cancelled** order additionally carries a "Perdido" marker (word plus icon, never colour alone), because that money is sunk and the progress block above has already netted it out of what counts as paid. The `{n} pedidos` shape carries the marker whenever any of its lines names a cancelled order: it is the one shape that cannot name the order, so the marker is the only warning left on the collapsed row. The marker and the "Sin asignar" chip never truncate away with a long product name.
  - **Breakdown.** A payment carrying two or more declarations offers one level of in-place disclosure listing each line and its amount, read-only (`BR-04-31`). A payment with a single declaration renders no toggle at all: the collapsed row already says everything the panel would, which is every payment in the collection today.
  - **True count and "see all".** The card's header count is the true total, not the number of rows the query's cap let through, and when rows are withheld the card offers a real control ("Ver los {total} pagos") that fetches the rest, moves focus to the first revealed row, and announces the new "{shown} de {total} pagos" in a text-only live region. It replaces a passive "y {n} más" sentence that read like a control and was not one.
  - **Why the main column, written down so it is not "fixed" later.** [`interface-patterns.md`](../../../design/interface-patterns.md) sends compact summaries to the right rail, and the rail is 320px wide, which leaves 278px of usable width inside the card padding. A row carrying date + a fixed-width 15-character order reference + product name + amount + two controls has roughly 7px left for the product name there, and even a two-line rail layout fits only about 64% of the collector's real product names against about 91% in the main column. Moving this list to the rail would truncate away the exact thing the row exists to show. What went to the rail is the half of this feature that **is** a compact summary: the progress block (`FR-04-59`).

- `FR-04-59`: **The store detail aside must render a payment progress block per currency.** _(Added 2026-08-10, denominator amended the same day.)_ Each block shows, in this order: a headline that reads "Falta {amount}" when a debt is live, an "Al día" chip when the debt is exactly zero, or "A favor {amount}" in the success tone when the store holds money for the collector; the percentage; a progress bar; and the absolute pair "{paid} pagados de {committed} en pedidos activos". Two conditional lines can follow: "Perdido en cancelados {amount}", when part of what the collector paid this store is still declared against a cancelled order, which is the only figure on the page for money that left the collector's hands and bought nothing (`paidMinor` nets it out, the bar's pair never counted it because a cancelled pedido is not active, and the payments list shows the payment at face value with no hint that part of it died with its pedido); and the reconciliation line between the headline and the bar's pair, in whichever direction the gap opens, "Fuera de pedidos activos {amount}" or "A cuenta {amount}", both defined below. Currencies are ordered with the one carrying live debt first, and are never summed together. Directly above the blocks, a "Cancelados {amount}" row is rendered for any currency where "Total facturado" (which counts cancelled orders) exceeds the store's committed total (which does not), naming the difference instead of leaving the two figures silently disagreeing; a currency with no debt row at all is the case where the cancelled slice is the whole of "Total facturado", not the case where it is zero. Every figure is derived from what [FRD-05 · `FR-05-43`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#functional-requirements) computes; this FRD adds no money rules.

  **The bar measures the pedidos activos, and the headline does not. Written down so it is not "unified" later.** The bar's ratio is `activePaidMinor / activeCommittedMinor`: money declared against the pedidos activos, over what those pedidos cost. The headline "Falta {amount}", the "Registrar pago" gate (`FR-04-56`), the "Por tienda" order view and the dashboard all keep reading the lifetime `debtMinor`, unchanged.

  > **Review note (2026-08-20, `ADR 0033`, [`FRD-05` · `FR-05-61`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#functional-requirements)):** The claim above is now only half true and describes today's shipped behavior, not the target once [`FRD-05` · `BP-01` · `WO-09`](../frd-05-order-payment-shipment/bp-01-order-domain-foundation/work-orders/wo-09-store-payment-assignment-and-open-order-debt.md) ships. `FR-05-61` scopes the collector-facing debt figure to the store's **open** orders, and `WO-09`'s own Module Structure moves both the headline ("Debes {amount}" → "Pendiente en pedidos abiertos {amount}") and the "Por tienda" order view (`StoreGroupHeader.tsx`) to that new `openOrderDebtMinor`, the same scope the bar already reads. Only the "Registrar pago" gate (`FR-04-56`) and the payment-validation ceiling stay on the lifetime `debtMinor` (`FR-05-63`), on purpose: a late payment against an already-delivered order must still be accepted. `WO-09` does not list the dashboard among the surfaces it switches; this document does not assert the dashboard's own figure either way.
  - **Why the ratio was narrowed.** The lifetime ratio accumulates and ends up settled, so it converges on 100% in any store with history, no matter what is owed right now. Measured on the collector's real data the day this shipped: the largest live debt (2,355.00 PEN) read **85%** while a debt 25 times smaller read **99%**, and the two bars looked the same. Scoped to the pedidos activos the same five rows read 39 / 13 / 20 / 23 / 56, which is the range the collector actually needs.
  - **Why `debtMinor` was NOT narrowed with it.** `debtMinor` is the ceiling `createStorePayment` enforces ([FRD-05 · `FR-05-43`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#functional-requirements)) and the gate on "Registrar pago". Narrowing it would refuse, and grey out, a payment against a pedido that was delivered without being fully paid, and would make this page disagree with the dashboard about how much the collector owes. Only the ratio moved. **Reviewed 2026-08-20 (`ADR 0033`):** this reasoning still holds for `debtMinor` itself, the gate and the ceiling, exactly as written. It stops being the whole story once [`FRD-05` · `FR-05-61`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#functional-requirements) ships: the **headline** then moves too, to its own new `openOrderDebtMinor` figure, distinct from `debtMinor`. "Only the ratio moved" describes the state before that change, not after it.
  - **No pedidos activos, no bar.** With `activeCommittedMinor === 0` the block keeps its headline (so the collector still gets "Al día" or "Falta {amount}"), drops the bar and the percentage, and its caption reads "Sin pedidos activos". A filled track over a zero denominator claims "100% of nothing" in a shape indistinguishable from real progress. **This is the ordinary case, not an edge one**: 112 of the collector's 122 store/currency pairs are here, all of them settled, and the block still renders in all 120 stores, so nothing is lost from the page.
  - **"Fuera de pedidos activos {amount}" and "A cuenta {amount}".** _(Second direction added 2026-08-10. **Reviewed 2026-08-20, `ADR 0033`:** this line is retired once [`FRD-05` · `FR-05-61`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#functional-requirements) ships, see the note below.)_ One line under the bar naming the difference between the headline (the whole debt) and what the bar's pair accounts for, in whichever direction it opens. Rendered only alongside a bar: with no bar, "Sin pedidos activos" already says the whole debt sits outside the active set.
    - **Positive, "Fuera de pedidos activos {amount}":** money owed on a pedido that has already been delivered. Without it the headline could say "Falta 2,625.00" above a pair adding up to 2,355.00. Zero cases today (the data cleanup of 2026-08-10 settled the last 16 completed orders that carried a balance); one delivery on a half-paid pedido away from returning.
    - **Negative, "A cuenta {amount}":** money already handed over that the bar's pair does not count, net of any balance still owed on a delivered pedido. This is the direction an allocation-less payment creates, and declaring nothing is a first-class submit in the pago sheet, "on account" ([FRD-05 · `FR-05-42`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#functional-requirements)): one pedido activo of 250.00 with nothing declared, plus a pago of 100.00 on account, reads "Falta 150.00" above "0.00 pagados de 250.00 en pedidos activos", and until this line existed nothing on the page named the 100.00 that reconciles the two. Zero cases today, and one pago away.
    - **Suppressed on a store in credit (`debtMinor < 0`) only.** There the headline is already "A favor {amount}" and a second, larger credit-shaped figure beside it would be two answers to "how much of mine are they holding?". Stated plainly because the earlier wording claimed more than it delivered: the "A favor" figure is the net position, not the arithmetic complement of the bar's gap, so this is a deliberate choice to keep one credit figure on the block, not a proof that the gap is covered.
    - **Superseded by `ADR 0033` (reviewed 2026-08-20, pending [`FRD-05` · `BP-01` · `WO-09`](../frd-05-order-payment-shipment/bp-01-order-domain-foundation/work-orders/wo-09-store-payment-assignment-and-open-order-debt.md)):** both directions of this line exist only to explain a gap between a lifetime headline and a bar scoped to pedidos activos. Once the headline itself moves to the open-order figure ([`FRD-05` · `FR-05-61`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#functional-requirements)), that gap closes structurally and this line is **retired**, not merely relabeled. What replaces the negative ("A cuenta") case is the store's own **dinero sin asignar** / **unassigned money** figure (`unassignedStoreMoneyMinor`, see the glossary entry), shown as its own line rather than netted into the debt figure. The positive ("Fuera de pedidos activos") case has no dedicated store-detail replacement in the current work orders: `WO-09` computes the equivalent `unrecordedPaymentsMinor` only to justify retiring this line, not as a rendered figure on this page; the collector-facing diagnostic for it is the dashboard's "pagos que no registraste" (`FR-06-28`, a different scope, owned by `FRD-06 · WO-07`).
  - **A currency with nothing committed and nothing paid** renders no block at all, only the conditional "Perdido en cancelados" line if it applies. This is what a store is left with when its last standing order is cancelled and its payment is kept as lost (`BR-05-15`). Distinct from "settled with nothing active", which does keep its "Al día" chip: there the collector is square with a store they still have a history with, here there is no standing left to be square about.
...
- `BR-04-31`: **The payment breakdown on the store detail card is read-only.** _(Added 2026-08-10.)_ The panel a multi-declaration payment opens (`FR-04-58`) lists each declaration and its amount and offers no control to edit or remove an individual `PaymentAllocation`. Editing one line is a third level of disclosure on a card whose whole purpose is answering a one-line question, and it would sit directly next to a delete that means something else entirely (`BR-04-30`, whole-payment). Per-allocation edits stay where the order that owns them lives ([FRD-05 · `BR-05-10`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#business-rules)). The read-only panel does carry one signal beyond the numbers: a line declared against a cancelled order is marked "Perdido", the same vocabulary the order detail and the dashboard already use for that money ([FRD-05 · `BR-05-15`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#business-rules)).
...
### `AC-04-32` The progress block agrees with the payments list underneath it

- Given a store where part of what the collector paid is still declared against a cancelled order
- When the aside's payment progress block renders
- Then it shows the absolute paid/committed pair rather than a bare percentage, never reads 100% while any balance is live, and names the sunk amount on its own "Perdido en cancelados {amount}" line, so the figure above never contradicts the sum of the payments listed below

### `AC-04-33` A payment row says what it was for, and deleting a sunk one moves nothing

- Given the "Pagos a esta tienda" card
- When a row renders
- Then it names the order reference and, when the declaration points at one, the product, without any expanding; a declaration against a cancelled order is additionally marked "Perdido"
- And when the collector deletes a payment entirely declared against a cancelled order, the aside's figures do not move, because the server's own paid figure already excludes that money
...
| `listAllStorePayments`        | `unauthorized`, `server_error` (`FR-04-58`, added 2026-08-10). A failure here is **partial**: the rows already on screen stay, the card shows an inline `role="alert"` line and relabels its control "Reintentar". It never replaces the section with an error state, because the 20 valid rows it already has are not what failed. |
...
- **store-level payments (added 2026-08-08, `FR-04-56`/`FR-04-58`; extended 2026-08-10):** `store_payment_sheet_opened`, `store_payment_registered`, `store_payment_deleted`, `store_payment_breakdown_opened`, `store_payments_all_loaded`. These are fired from store-detail entry points defined by this FRD, but the sheet's own submission mechanics belong to [FRD-05 · `FR-05-42`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md#functional-requirements), which is why FRD-05's own Analytics section also references this namespace instead of duplicating the events.
...
- `src/app/[locale]/(app)/stores/[slug]/_components/StorePaymentProgressRows.tsx` (the payment progress block that replaced the earlier `StoreDebtSummaryRows.tsx`)
...
- `src/app/[locale]/(app)/stores/[slug]/_components/StorePaymentRow.tsx`
