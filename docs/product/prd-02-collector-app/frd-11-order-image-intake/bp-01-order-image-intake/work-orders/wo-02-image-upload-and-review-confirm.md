...
- **Idempotency marker.** `saveOrderFromDraftAction` (`imageIntakeSaveAction.ts`) builds a `[image-intake:<sha256-prefix>]` marker from the signing collector, the matched store, the order date, currency, total, and the product/price signature, writes it into the order's `note`, and checks for an existing order with that marker before creating a new one. A resubmission of the same reviewed draft (a double-tap, a retried request) returns the existing order instead of creating a duplicate, following the precedent already used by `chat-load.ts`.
