...
- `User.budgetAmount` is stored in **minor units**, matching every other money column in the app (`Order.totalCost`, `OrderPayment.amount`), so cross-domain consumers never have to ask which unit a money field carries. The collector still budgets in whole currency units: `src/lib/user-settings/budgetAmount.ts` is the only conversion boundary (`×100` on save, `÷100` on prefill), and the whole-unit contract is enforced twice — a Zod multiple-of-`100` refinement and the `user_budgetAmount_minor_units_check` database constraint. The ceiling (`999_999_900` minor) is set by the `INTEGER` column, not by product preference.
...
- Store listing entry URL contract:
  ...
  - cross-surface rule: any **future** in-app link to the same listing (for example dashboard CTAs under FRD-06) must call the **same shared URL builder** as the shell so behavior stays consistent; see [FRD-06 · Cross-domain notes](../../frd-06-dashboard/frd-06-dashboard.md#cross-domain-notes)
...
- future dashboard/reporting consumers of base currency and budget defaults from [`FRD-06`](../../../frd-06-dashboard/frd-06-dashboard.md)
