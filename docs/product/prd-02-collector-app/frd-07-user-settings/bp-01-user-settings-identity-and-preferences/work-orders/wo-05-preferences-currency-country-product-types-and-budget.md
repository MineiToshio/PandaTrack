...
- **Inline explicit confirmation** (not a modal, not autosave) when the user changes **base currency** from the last persisted value:
  ...
  - `Cancelar` (`preferences.collector.currency.cancel`) discards the pending value; only `Guardar` (`preferences.collector.currency.save`) commits it via `updateCurrencyAction({ baseCurrencyCode })`, which persists the new base currency and flags affected orders for FX reconciliation in the same transaction, then returns `pendingFxOrderCount`,
  ...
  - skipping the shortcut leaves those orders flagged for manual per-order updates (`FR-07-32`); they surface in the orders list banner and `?fxPending=true` filter.
...
- **Base currency change** on successful save after the inline confirm: `updateCurrencyAction({ baseCurrencyCode })` persists the new base currency and, **only when the base currency actually changed** (`current.baseCurrencyCode !== trimmed`), calls `flagOrdersForFxReconciliation` (via `applyBaseCurrencyChange`, same transaction) to flag affected `Order` records (`needsExchangeRateUpdate = true` where currency differs from the new base, `false` where it matches). It never mutates `exchangeRate`. The action returns `pendingFxOrderCount`; when it is `> 0` the pane surfaces an **optional** shortcut `"Actualizar tasas · {n} pedidos →"` to `/{locale}/orders?fxPending=true`, where the FX reconciliation flow (owned by [**FRD-05 · BP-02 · WO-07**](../../../frd-05-order-payment-shipment/bp-02-order-workspace-and-list-experience/work-orders/wo-07-currency-reconciliation-filter-and-bulk-fx-reconciliation.md)) reads the flag and lets the collector apply rates per currency pair. No navigation is forced; when the collector skips the shortcut the orders stay flagged and the orders list banner guides reconciliation at their own pace.
...
- When the user changes base currency and confirms with `Guardar`, the base currency is saved and affected orders are flagged for reconciliation (no `exchangeRate` is mutated); when foreign-currency orders remain, the pane exposes the optional `"Actualizar tasas · {n} pedidos →"` shortcut to `/{locale}/orders?fxPending=true`, where the FX reconciliation flow surfaces the eligible currency pairs to apply or skip.
- When the user takes the reconcile shortcut, they land on the orders list filtered to `?fxPending=true`; when they skip it, the orders stay flagged and the orders list banner/`?fxPending=true` filter guides the collector to reconcile at their own pace.
