...
- `FR-07-32`: When the user changes their saved base currency, the settings flow must present an explicit confirmation step that explains the change affects currency reconciliation for orders impacting current and future budget periods, offers a path to bulk reconciliation by currency pair, and warns that skipping reconciliation now requires manual per-order updates later. **Implemented** as a two-path confirmation footer in `CurrencyModal`, both backed by the same `updateCurrencyAction({ baseCurrencyCode, saveFxRates })` server action. When the base currency actually changes, the action persists the new base **and** calls `flagOrdersForFxReconciliation(userId, newBase)` (in `src/lib/data/orders/orderMutations.ts`), which sets `Order.needsExchangeRateUpdate = true` on every order whose currency differs from the new base (and clears it on orders that now match). This only flags — it never mutates any order's `exchangeRate` (the deliberate "no silent bulk rate mutation" rule; the collector reconciles per-row). The action returns `redirectToFxReconcile: saveFxRates`. Path A ("Save and update exchange rates") sets `saveFxRates: true`, so on success the modal navigates the collector to `/{locale}/orders`, where the FX reconciliation modal lets them preview and apply rates per currency pair (clearing the flag as they go). Path B ("Save without updating") sets `saveFxRates: false` and closes after saving — the orders stay flagged so the orders-list banner surfaces them later.
...
- And the user can choose to save and be routed to `/{locale}/orders` to reconcile by currency pair, or save and reconcile manually later
- And in both paths the action only persists the new base currency; no `Order` records are modified by the settings flow
...
- **Server actions:** `savePreferencesAction` (autosaving patch for country / product types / budget, `FR-07-35`), `updateCurrencyAction` (two-path base-currency change, `FR-07-32`), `updateLanguageAction` (`NEXT_LOCALE` cookie, `FR-07-36`), and `getPreferencesSnapshotAction` (read-only preferences reload). Theme is client-only (`ThemeContext`), persisted to `THEME_STORAGE_KEY` local storage — no server action.
- **States:** autosave indicator (`saving / saved / error`) with optimistic local apply + revert-on-failure; budget inputs disabled until a base currency is set (`baseCurrencyCode == null`); the currency change opens `CurrencyModal` instead of autosaving; mobile-only full-width `"Cerrar sesión"` ghost-destructive button at the foot of the pane.
