...
- `FR-07-32`: When the user changes their saved base currency, the settings flow must present an explicit confirmation step (not an autosave) that warns the change does not convert historical data and requires manual per-order reconciliation later. **Implemented** as an inline explicit-confirm on the `Preferences` pane, not a modal: picking a new value in the base-currency select stages a pending choice and reveals `"Guardar"` / `"Cancelar"` with the hint `"No convierte tus datos anteriores."`; only `"Guardar"` commits it via `updateCurrencyAction({ baseCurrencyCode })`. When the base currency actually changes, the action persists the new base **and** calls `flagOrdersForFxReconciliation(userId, newBase)` (via `applyBaseCurrencyChange` in `src/lib/data/user-settings/userSettingsMutations.ts`; the flag helper lives in `src/lib/data/orders/orderMutations.ts`) in the same transaction, which sets `Order.needsExchangeRateUpdate = true` on every order whose currency differs from the new base (and clears it on orders that now match). This only flags — it never mutates any order's `exchangeRate` (the deliberate "no silent bulk rate mutation" rule; the collector reconciles per-row). The action returns `pendingFxOrderCount` (the number of foreign-currency orders still needing a rate). When that count is `> 0`, the pane surfaces a single **optional** shortcut — `"Actualizar tasas · {n} pedidos →"` — linking to `/{locale}/orders?fxPending=true`, where the existing FX reconciliation flow lets the collector preview and apply rates per currency pair (clearing the flag as they go). No navigation is forced; when the collector skips the shortcut the orders stay flagged so the orders-list banner surfaces them later. (Replaces the former two-path `CurrencyModal` "save without / save and update" footer — a single save plus a conditional shortcut is enough because the reconcile entry point already lives on `/orders`.)
...
- And the settings flow only persists the new base currency and sets the reconcile flag on affected orders — it never converts amounts or mutates any order's exchange rate
