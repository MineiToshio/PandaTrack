...
- `FR-06-07`: The dashboard must surface total disbursed spend for the **current calendar month** — the sum of `OrderPayment.amount` whose `paymentDate` falls in the current calendar month, including partial and advance payments.
- `FR-06-08`: The dashboard must provide a monthly disbursed-spend chart across a configurable date range.
...
- `BR-06-04`: Spend is measured as **disbursed cash-out** by `OrderPayment.paymentDate`. A partial or advance payment counts at the moment it is paid, never deferred to a later period.
...
- `BR-06-07`: Orders in `CANCELLED` status are excluded from obligation, committed-value, collection-state, and activity rollups.
...
| Desembolsado este mes       | Σ `OrderPayment.amount` with `paymentDate` in the current month                                                                                                          | `OrderPayment`                                                    | current calendar month                   |
| Gasto por mes (chart)       | Σ `OrderPayment.amount` grouped by `paymentDate` month                                                                                                                   | `OrderPayment`                                                    | selected range                           |
...
- Spend means disbursed cash-out by payment date, including partial payments; "committed" (order total) is a distinct, separately labeled concept.
