...
- `BR-06-08` **(revised 2026-08-08, store-level payments):** Outstanding balance per order is `Order.totalCost` minus its own `Order.allocatedAmountMinor` (the sum of `PaymentAllocation` rows declared against it), **clamped at zero in presentation** — never shown negative for a single order, consistent with the payment rules in [`FRD-05`](../frd-05-order-payment-shipment/frd-05-order-payment-shipment.md). This clamp is **order-level presentation only** and does not extend to the store: a store's own debt (`Σ totalCost` of its non-cancelled orders `− Σ` its payments, per currency) is deliberately **not** clamped, because a negative value there is real money the store holds on the collector's behalf ("a favor") and clamping would erase that signal (`FR-05-43`, `docs/design/decisions/0025-store-level-payments-declared-allocations.md`).
...
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ---------------------------------------- |
| A pagar este mes             | Σ outstanding balance of orders with `expectedDeliveryFrom` in the current month, plus Σ outstanding of all overdue orders (`expectedDeliveryFrom < today`, balance > 0)                                    | `Order`, `PaymentAllocation`                                           | current calendar month + overdue fold-in |
| Próximos meses               | Per-month Σ outstanding balance bucketed by `expectedDeliveryFrom` month                                                                                                                                    | `Order`, `PaymentAllocation`                                           | next 2+ calendar months                  |
| Deuda viva total             | Σ outstanding balance across all non-cancelled orders                                                                                                                                                       | `Order`, `PaymentAllocation`                                           | all-time                                 |
| Deuda sin fecha              | Σ outstanding balance of orders with no `expectedDeliveryFrom`                                                                                                                                              | `Order`, `PaymentAllocation`                                           | all-time                                 |
...
| Pagado vs pendiente          | Committed (Σ `totalCost`) split into paid (Σ payments) and outstanding (deuda viva) across non-cancelled orders                                                                                             | `Order`, `PaymentAllocation`                                           | all-time                                 |
| Deuda viva (tendencia)       | Outstanding balance at each month-end over the selected range                                                                                                                                               | `Order`, `PaymentAllocation`                                           | selected range                           |
