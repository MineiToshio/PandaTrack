...
| `humanReadableId`      | `String` (unique per `userId`)        | Format: `DLV-YYYYMMDD-NN`. Generated by `generateDeliveryHumanReadableId()` in `src/lib/deliveries/deliveryIdentifier.ts`. Uniqueness is scoped per user, matching the per-user daily sequence.                                                                                                                                                                                                                 |
...
| `exchangeRateBaseCode` | `String?`                             | The base currency `exchangeRate` converts INTO, written whenever a rate is persisted and `null` when there is no usable rate; mirrors `Order.exchangeRateBaseCode`. It is what makes the `FR-08-10a` pending state **derived** instead of stored: a base-currency change writes nothing, and per-delivery edit is the reconciliation path because saving stamps the current base alongside the rate (ADR 0024). |
