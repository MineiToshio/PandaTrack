...
- Action display: the raw stable action key in mono (for example `store.remove`), with a localized `title` describing it. The 10-key vocabulary already lives in `src/lib/data/admin/adminAuditVocabulary.ts`.
