...
> **Amended 2026-07-27, owner-approved.** The derived fifth row was originally a "flag candidate / suggested removal" whose review offered flag and unflag. The manual flag control and the `FLAGGED` store status no longer exist: the public "has reports" notice is now derived from open reports, so the row is reframed as a **report cluster** whose review resolves or dismisses the reports themselves. This work order is amended in place to describe the final behavior. The console files for that change are delivered by [PRD-02, FRD-04 · BP-01 · WO-13](../../../../prd-02-collector-app/frd-04-store-domain/bp-01-store-public-trust-system/work-orders/wo-13-derived-report-notice-and-flag-removal.md), not by this slice, because the `StoreStatus` enum value cannot be dropped while the console still references it. See [ADR 0019](../../../../../design/decisions/0019-derived-trust-signals-moderation-status-lifecycle-only.md).

## Summary

Vertical slice that builds the moderation inbox: a server-only aggregate read model that gathers the four persisted pending categories and derives a fifth row type (the **report cluster**) per the `FR-02-05` rule when a store accumulates 2 or more open reports, shaping each item's per-type review payload, a master-detail presentation (queue plus review pane on desktop, stacked queue plus full-width detail on mobile), the five per-type review panels (pending store, report, report cluster, change request including its drift variant, product-type suggestion), and the invocation of the server actions owned by PRD-02, [FRD-04](../../../../prd-02-collector-app/frd-04-store-domain/frd-04-store-domain.md) from each review. This is the core value of the first release: the administrator reviews and acts without leaving the console.
...
- Deriving the report-cluster row when a store accumulates 2 or more open reports, collapsing that store's individual report rows into the single derived row. The threshold is the named constant `STORE_REPORT_CLUSTER_THRESHOLD` in `src/lib/constants.ts`, not a repeated literal, and it is deliberately **not** the same constant as `STORE_REPORT_NOTICE_THRESHOLD` (1), which drives the public store-detail notice (PRD-02, FRD-04 `FR-04-43`).
- Impact ordering across five tiers, highest first: report cluster, open reports, pending stores, change requests, product-type suggestions; oldest first within each tier.
...
- Invoking the shared removal-reason modal owned by FRD-04 from the pending-store, report, and report-cluster reviews; the modal is promoted to a location importable from both the store route subtree and the admin route subtree.
- Sensitive fields read through the admin-only path, never through the public governance read model.
- Analytics for opening an inbox item (`admin_inbox_item_opened` with an `item_type` property, one of `report`, `report_cluster`, `pending_store`, `change_request`, `product_type`), emitted from a small client island.
- Unit tests for the aggregate, its ordering, its report-cluster derivation and collapse, and the per-type payload shaping; a narrow, skippable admin E2E for the inbox render (empty and populated) and one or two review flows, matching the acceptance criteria below.

## Out of Scope

- The moderation server actions themselves (approve, remove, resolve, dismiss, apply, reject for both change requests and product types) and the removal-reason modal's definition; owned by PRD-02, [FRD-04](../../../../prd-02-collector-app/frd-04-store-domain/frd-04-store-domain.md). WO-02 invokes these; it does not build them. Promoting the existing removal modal to a shared location is a placement move, not a redefinition of the mutation.
...
- `FR-02-16`: Report-cluster review with per-report resolve and dismiss actions, plus remove.
...
- **Aggregate composition.** `moderationQueueQueries.ts` reads pending stores (they are `PENDING`, outside `PUBLIC_VISIBLE_STORE_STATUSES`), open reports and pending change requests per store through the admin DALs, and pending product-type requests globally through `getAdminPendingStoreProductTypeRequests()`. It groups open reports per store; when a store's open-report count reaches `STORE_REPORT_CLUSTER_THRESHOLD` (2), it emits one derived report-cluster row and drops that store's individual report rows. The same read shapes both the queue row and the selected item's review payload so the two never drift apart.
- **Selection routing.** The inbox is a Server Component reading `searchParams.item`. The value is `<type>:<id>` where `type` is one of `report`, `report_cluster`, `pending_store`, `change_request`, `product_type`, because the ids live in different tables. A `report_cluster` id is the store id, since the row is a view over that store's open reports rather than a record of its own. No explicit `item` selects the top row on desktop (auto-preview) and renders the bare queue on mobile.
...
- **Constants.** `STORE_REPORT_CLUSTER_THRESHOLD = 2` in `src/lib/constants.ts`, alongside and distinct from `STORE_REPORT_NOTICE_THRESHOLD = 1`. Both are imported through one shared predicate so the console and the public store detail cannot drift on what counts as reported (PRD-02, FRD-04 `FR-04-43`).
...
- Add `INBOX_ITEM_OPENED: "admin_inbox_item_opened"` to `POSTHOG_EVENTS.ADMIN` in `src/lib/constants.ts`, emitted from a small client island keyed on the selected item, carrying `{ item_type }` (`report` / `report_cluster` / `pending_store` / `change_request` / `product_type`), following the `AdminSpaceEnteredCapture` precedent. This is a view event, so it is client-side; the mutation events stay inside the FRD-04 actions.
...
- PRD-02, FRD-04 · [WO-09](../../../../prd-02-collector-app/frd-04-store-domain/bp-01-store-public-trust-system/work-orders/wo-09-store-approval-and-removal.md) through [WO-12](../../../../prd-02-collector-app/frd-04-store-domain/bp-01-store-public-trust-system/work-orders/wo-12-product-type-request-approval.md) (implemented): the eight callable moderation server actions, the admin DALs, the `StoreRemovalModal`, `StoreRemovalReason`, the `SUPERSEDED` status, and `PUBLIC_VISIBLE_STORE_STATUSES`.
- [PRD-02, FRD-04 · BP-01 · WO-13](../../../../prd-02-collector-app/frd-04-store-domain/bp-01-store-public-trust-system/work-orders/wo-13-derived-report-notice-and-flag-removal.md) delivers the report-cluster framing described above, including the `report_cluster` item type, the renamed threshold constant, and the removal of the flag/unflag actions this work order originally invoked.

## Testing Notes

- Unit tests target the pure aggregate logic in `moderationQueueQueries.ts`: composition of the four categories, report-cluster derivation and report-row collapse at the threshold, five-tier impact ordering with oldest-first within a tier, and per-type payload shaping. These do not require a seeded database when the composition is factored into pure functions over the DAL results.
...
- A store with 2 or more open reports appears once in the queue as a single report-cluster row, and not as its individual report rows (`AC-02-02`, `FR-02-05`).
...
- Opening a report-cluster item renders its review with per-report resolve and dismiss actions plus remove, and with no flag control anywhere (`AC-02-09`).
