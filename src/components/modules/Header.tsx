...
import LangToggle from "@/components/core/LangToggle";
...
export default function Header({ locale, pathname, drawerOpen, onOpenDrawer, burgerButtonRef }: HeaderProps) {
  ...
  return (
    <header
      className={cn(
        "border-border bg-background/95 supports-backdrop-filter:bg-background/85 sticky top-(--app-banner-offset,0px) z-30 shrink-0 border-b backdrop-blur",
        "h-[var(--header-h)] lg:h-[var(--header-h-desktop)]",
      )}
    >
      <div
        className={cn(
          "mx-auto flex h-full w-full items-center justify-between gap-3 px-4 sm:gap-4 sm:px-6 lg:px-8",
      ...
      >
        <div className="flex min-w-0 flex-1 items-center gap-2 lg:flex-initial lg:gap-4">
          ...
          <div className="flex min-w-0 flex-1 flex-row items-center gap-1.5 sm:gap-2">
            {displayBreadcrumbs.length > 0 && (
              <>
                <nav
                  aria-label={t("accessibility.breadcrumbNavigation")}
        ...
        <div className="hidden shrink-0 items-center gap-2 sm:gap-3 lg:flex">
          <LangToggle
            ariaLabel={t("accessibility.languageNavigation")}
