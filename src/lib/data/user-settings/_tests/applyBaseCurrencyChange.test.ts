...
const { prismaMock, flagMock, flagDeliveriesMock, validationMock } = vi.hoisted(() => ({
  prismaMock: { $transaction: vi.fn() },
  flagMock: vi.fn(),
  flagDeliveriesMock: vi.fn(),
  validationMock: {
...
vi.mock("@/lib/data/deliveries/deliveryMutations", () => ({
  flagDeliveriesForFxReconciliation: flagDeliveriesMock,
}));
...
describe("applyBaseCurrencyChange", () => {
  ...
  it("flags orders and deliveries for FX reconciliation on the same transaction as the currency write", async () => {
    ...
    // The currency scalar was written and both flags were set within the same tx client, so
    ...
    expect(flagDeliveriesMock).toHaveBeenCalledTimes(1);
    expect(flagDeliveriesMock).toHaveBeenCalledWith("user-1", "USD", fakeTx);
    expect(prismaMock.$transaction).toHaveBeenCalledTimes(1);
  });

  it("does not flag orders or deliveries when the base currency is unchanged", async () => {
    ...
    expect(flagDeliveriesMock).not.toHaveBeenCalled();
  });

  it("does not flag orders or deliveries when the preference patch fails validation", async () => {
