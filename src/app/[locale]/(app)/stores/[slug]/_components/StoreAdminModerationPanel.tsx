...
import { Ban, CheckCircle, Flag, ShieldCheck } from "lucide-react";
...
/**
 * Admin-only moderation controls for a store's own state (approve / flag / unflag / remove). The
 * inline transitions are optimistic: the panel reflects the new status immediately and reverts on
 * failure with a toast. Removal uses Optimistic Confirmation — the modal closes synchronously on
 * submit and the panel coordinates the navigation-away (the store 404s) or the rollback toast.
 */
export default function StoreAdminModerationPanel({
  ...
  // without a prop-sync effect — this also corrects the optimistic unflag guess.
  ...
  const handleFlag = () => {
    void runInlineTransition("FLAGGED", () => flagStoreAction({ slug: storeSlug, locale }), "toasts.flagged");
  };

  const handleUnflag = () => {
    // The optimistic guess is PENDING; the server derives the real prior state (APPROVED vs
    // PENDING) and the revalidated prop corrects it on the next render.
    void runInlineTransition("PENDING", () => unflagStoreAction({ slug: storeSlug, locale }), "toasts.unflagged");
  };
    ...
    <div className="flex flex-col gap-2">
      ...
      {(status === "PENDING" || status === "APPROVED") && (
        <Button
          ...
          fullWidth
          className="justify-start"
          leadingIcon={<Flag size={16} aria-hidden="true" />}
          onClick={handleFlag}
          disabled={isPending}
        >
          {t("flagCta")}
